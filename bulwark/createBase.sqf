...
_bulMon setObjectTexture [0, "overview.jpg"];
...
/* dzn: Loot spawn */
DZN_WRECK_LOOT_POINTS = [];
DZN_WRECKS = [];
for "_i" from 1 to (("par_wrecksNumber" call  BIS_fnc_getParamValue) + round(random 4)) do {
	private _wreck = (selectRandom [
		"_1489thChimAuto"
		, "Sentinel_AC_1489th_1"
		, "_1490thChimAuto"
		, "Valkyrie_FP_1490th_1"
		, "_RenegadeChimAuto_Brown_OP"
		, "Valkyrie_Possessed_0"
		, "LRMKIV_possessed_0_Brown"
		, "LRMKIV_1490th_1"
	]) createVehicle ([bulwarkCity, BULWARK_RADIUS * 0.25, BULWARK_RADIUS,1,0] call BIS_fnc_findSafePos);
	
	_wreck setVariable ["ace_cookoff_enableAmmoCookoff", false];
	_wreck setDir (random 360);
	_wreck lock true;
	_wreck setDamage 1;
	
	private _wreckPos = _wreck getPos [random [2,3,5], random 360];
	_wreckPos set [2,0.1];
	
	private _lootHolder = "WeaponHolderSimulated_Scripted" createVehicle _wreckPos;
	DZN_WRECK_LOOT_POINTS pushBack _lootHolder;
	DZN_WRECKS pushBack _wreck;
	
};

publicVariable "DZN_WRECKS";

[] execVM "dzn\ambientArtillery.sqf";

waitUntil { !isNil "dzn_fnc_StartVirtualFiremission" && !isNil "dzn_fnc_spawnShell" };
[ [bulwarkCity, BULWARK_RADIUS + 150, BULWARK_RADIUS + 1000], "Sh_82mm_AMOS", -5, 5, 15, 2 ] spawn dzn_fnc_StartVirtualFiremission;
