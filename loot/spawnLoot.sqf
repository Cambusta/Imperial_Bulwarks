...
/* Supply Drop */
[bulwarkCity, ["<t color='#00ff00'>" + "FILL AMMO", "supports\ammoDrop.sqf","",2,true,false,"true","true",4], "Valkyrie_RP_1490th_1"] remoteExec ["supports_fnc_supplyDrop", 2];


/* dzn LOOT */
{
	// Add gear to lootpoint or remove all gear in current loop
	_lootHolder = _x;
	if (random 100 <= ("par_wreckLootChance" call  BIS_fnc_getParamValue)) then {		
		switch (floor random 6) do {
			case 0: {
				_weapon = selectRandom LOOT_WEAPON_POOL;
				_ammoArray = getArray (configFile >> "CfgWeapons" >> _weapon >> "magazines");
